 ...
 */
static unsigned int des3_key_to_random(unsigned char *random, const unsigned char *key, unsigned int len)
{
 ...
 */
static int key_derivation_function(krb5_creds *creds, uint8_t *session_key)
{
    ...
    _kafs_dbg("aklog: Unable to derive strong DES key\n");
    return(1);
...
des3_discard_parity:
	if (length & 7) {
        _kafs_dbg("aklog: 3DES session key not multiple of 8 octets.\n");
        return(1);
	}
...
derive_key:
    return( key_derivation_function(creds, session_key) );

	/* Use as-is for single-DES [afs3-rxkad-k5-kdf-00 §4.1]. */
just_copy:
	if (length != 8) {
        _kafs_dbg("aklog: DES session key not 8 octets.\n");
        return(1);
	}
...
deprecated:
    _kafs_dbg("aklog: Ticket contains deprecated enc type (%d)\n",
		creds->keyblock.enctype);
    return(1);

not_supported:
    _kafs_dbg("aklog: Ticket contains unsupported enc type (%d)\n",
		creds->keyblock.enctype);
    return(1);
key_too_short:
    _kafs_dbg("aklog: Ticket contains short key block (%u)\n", length);
    return(1);
}
